...
const matcher = await readFile(join(ROOT, "extension/lib/matcher.js"), "utf8");
const results=[];
...
check("Workday masked MM/YYYY handling exists", matcher.includes("setMaskedMonthYear") && matcher.includes("One digit at a time"));
check("year-only has a deterministic MM/YYYY fallback", fieldMap.includes('parsed.month || "01"'));
